...
class GoogleGeoCoder():
    ...
    def __init__(self, api_key=None):
        ...
        self.url = 'https://maps.googleapis.com/maps/api/geocode/json?address={address}'

    def geocode(self, address):

        if self.api_key is not None and self.api_key.replace(' ', '') != '':
            self.url += self.url + '&key=' + self.api_key
        else:
            url = self.url

        try: 
            self.url = self.url.format(**{'address': address.decode('utf8')})
            logMessage(self.url)
            results = json.loads(NAM.request(self.url, blocking=True)[1].decode('utf8'))['results']
            print(self.url)
